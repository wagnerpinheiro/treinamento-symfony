...
abstract class BaseFuncionarios extends BaseObject  implements Persistent {
	 ...
	 */
	public function getNascimento($format = 'Y-m-d')
	{
	 ...
	 * @return     Funcionarios The current object (for fluent API support)
	 */
	...
	public function setDepartamentoId($v)
	{
		...
		if ($this->departamento_id !== $v) {
			...
			$this->modifiedColumns[] = FuncionariosPeer::DEPARTAMENTO_ID;
		}

		if ($this->aDepartamentos !== null && $this->aDepartamentos->getId() !== $v) {
	...
	public function setNascimento($v)
	{
		...
		if ( $this->nascimento !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = FuncionariosPeer::NASCIMENTO;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->departamento_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->nascimento = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = FuncionariosPeer::NUM_COLUMNS - FuncionariosPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aDepartamentos !== null && $this->departamento_id !== $this->aDepartamentos->getId()) {
			$this->aDepartamentos = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = FuncionariosPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseFuncionarios:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseFuncionarios:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseFuncionarios:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseFuncionarios:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				FuncionariosPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aDepartamentos !== null) {
				if (!$this->aDepartamentos->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aDepartamentos->getValidationFailures());
				}
			}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FuncionariosPeer::DEPARTAMENTO_ID)) $criteria->add(FuncionariosPeer::DEPARTAMENTO_ID, $this->departamento_id);
		if ($this->isColumnModified(FuncionariosPeer::NASCIMENTO)) $criteria->add(FuncionariosPeer::NASCIMENTO, $this->nascimento);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDepartamentoId($this->departamento_id);
	...
	/**
	 * Declares an association between this object and a Departamentos object.
	 *
	 * @param      Departamentos $v
	 * @return     Funcionarios The current object (for fluent API support)
	 * @throws     PropelException
	 */
	public function setDepartamentos(Departamentos $v = null)
	{
		if ($v === null) {
		...
		// If this object has already been added to the Departamentos object, it will not be re-added.
		if ($v !== null) {
			$v->addFuncionarios($this);
		}
	...
	/**
	 * Get the associated Departamentos object
	 *
	 * @param      PropelPDO Optional Connection object.
	 * @return     Departamentos The associated Departamentos object.
	 * @throws     PropelException
	 */
	public function getDepartamentos(PropelPDO $con = null)
	{
		if ($this->aDepartamentos === null && ($this->departamento_id !== null)) {
			$this->aDepartamentos = DepartamentosPeer::retrieveByPk($this->departamento_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aDepartamentos->addFuncionarioss($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseFuncionarios:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseFuncionarios::%s', $method));
	  }
